...
const NEW_CATEGORIES = [
  { key: 'citrus', label: '柑橘汁' }, { key: 'syrup/staple', label: '糖浆/常备' }, { key: 'liqueur', label: '利口酒' },
...
function displayIngredient(row) { return { ...row, nameLabel: row.name || '选择材料', ...unitView(row.unit) } }
...
Page({
  ...
  onIngredientChange(event) { const { index, field, value } = event.detail; const ingredients = this.data.form.ingredients.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item); this.sync({ ...this.data.form, ingredients }) },
  ...
  onPickName(event) {
    const index = event.detail.index; const ingredient = this.data.form.ingredients[index] || {}; const common = ingredient.category === 'citrus' ? ['柠檬汁', '青柠汁'] : ingredient.category === 'syrup/staple' ? ['糖浆', '蜂蜜糖浆', '肉桂糖浆'] : []
    const suggestions = [...common.map((name) => ({ name, category: ingredient.category, kind: 'replace' })), ...this.materials.map((item) => ({ ...item, kind: 'existing' }))]
    this.setData({ suggestionOpen: true, suggestionIndex: index, suggestions })
  },
  ...
  onSave() {
    ...
    const recipe = { ...built.recipe, ingredients: built.recipe.ingredients.map((item, index) => ({ ...item, materialId: item.materialId || idsByName[checked.form.ingredients.filter((row) => row.name && (row.unit === 'top-up' || Number(row.amount) > 0))[index].name] || '' })) }
