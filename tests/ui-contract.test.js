...
test('long-term ownership is controlled from detail and editor actions share one row', () => {
  const editor = fs.readFileSync(path.join(MINI, 'pages/material-edit/index.wxml'), 'utf8')
  ...
  assert.match(detail, /class="availability-row"[^>]*wx:if="{{detail\.canToggleOwned}}"/)
  assert.match(detail, /<switch[^>]*checked="{{detail\.owned}}"[^>]*bindchange="onToggleOwned"/)
